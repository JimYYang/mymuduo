                               ...
                               highWaterMark_(64*1024*1024) // 64M                               
{
    // 下面给Channel设置相应的回调函数
    // 如果channel感兴趣的事件发生
    // poller会给channel通知 channel会回调相应的回调函数
...
/**
 * 发送数据 应用写得快 内核发送数据慢 需要把待发送数据写入缓冲区 而且设置了水位回调
*/
void TcpConnection::sendInLoop(const void *data, size_t len)
{
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0)
    {
        ...
        if (nwrote >= 0)
        {
            ...
            {
                // 既然这里数据全部发送完成 就不用再给channel设置epollout事件了
    ...
    // 说明这一次write 并没有把数据全部发送出去 剩余的数据需要保存到缓冲区当中
    ...
    if (!faultError && remaining > 0)
    {
        ...
        if (!channel_->isWriting())
        {
            channel_->enableWriting(); // 这里一定要注册channel的写事件 否则poller不会给cahnnel通知epollout
        }
